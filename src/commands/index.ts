...
export function registerBot(bot: Bot<MyContext>) {
  ...
  bot.command("start", async (ctx) => {
    ...
    try {
      ...
      rateText =
        "\n\nАктуальные обменные курсы АТБ на сегодня:\n" +
        `USD: ${nf.format(rates.USD)} ₽\n` +
        `EUR: ${nf.format(rates.EUR)} ₽\n` +
        `JPY: ${nf.format(rates.JPY)} ₽`;
    } catch (e) {
      console.error("Не удалось получить курсы АТБ", e);
    }
    await ctx.reply(
      "Привет! Я помогу рассчитать стоимость ввоза авто из Японии.\n" +
